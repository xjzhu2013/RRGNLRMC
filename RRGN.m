function [out,X]=RRGN(A,X0,U,S,V,gtol,maxitr,r,ind,row,col)
tic;
X=X0;
count=0;
...
for k=1:maxitr
    ...
    theta=0.1;
    delta=1.0e-4*(normGrad);
    [M,Up,Vp,count,Dir]=CG2(G,U,V,ind,row,col,delta,theta,count,imax,Mg,Upg,Vpg,Grad);      
    GDir=sum(sum(G.*Dir));
